...
namespace SecCsChatBotDemo
{
    ...
    public class MessagesController : ApiController
    {
                ...
                if(cacheList.luisId != null && cacheList.intentId != null && cacheList.entitiesIds != null){
                    ...
                    for (int i = 0; i < LuisDialogID.Count; i++)
                    {
                        ...
                        for (int n = 0; n < dlg.Count; n++)
                        {
                            ...
                            //string dlgGroup = dlg[n].dlgGroup;
                                    
                            Debug.WriteLine("* dlgId : " + dlg[n].dlgId);
                            //Debug.WriteLine("* dlgGroup : " + dlg[n].dlgGroup);
                            Debug.WriteLine("* dlgType : " + dlg[n].dlgType);

                            if (dlgType == "2") //TEXTDLG
                            {
                                List<TextList> text = db.SelectDialogText(dlg[n].dlgId);
                    ...
                    for (int i = 0; i < luisEntityCount; i++)
                    {
                        ...
                        Debug.WriteLine(":::::::::::::::::::::::::::::::: " + (string)Luis["entities"][i]["entity"]);
                        //entities = entities + (string)Luis["entities"][i]["entity"];
                        if(i==0){
                            entities = (string)Luis["entities"][i]["entity"];
                        }else{
                            entities = entities +","+(string)Luis["entities"][i]["entity"];
                        }
        ...
        private static async Task<JObject> GetIntentFromBotLUIS(string Query)
        {
            ...
            RequestURI[0] = "https://westus.api.cognitive.microsoft.com/luis/v2.0/apps/442508c8-6cd9-4bc4-9400-de2c70e33ce2?subscription-key=042998268da14ffc9307559f69a230dc" + "&verbose=true&timezoneOffset=0&q=" + Query;
